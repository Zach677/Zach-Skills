...
This skill is the Task 1 shell for a future bootstrap flow that will sit above `bin/zach-mobile-init`. It defines the interviewing, capability detection, and confirmation choreography, plus the structured JSON handoff contract that later tasks will implement.
...
1. Detect the local capabilities: `git`, `gh`, `gh auth status`, `mise`, `tuist`, and `tuist auth whoami`. Summarize what is available so the user understands the options.
...
8. If Tuist Cloud creation is confirmed, later tasks will run `tuist project create <full_handle>`. If cache setup is confirmed, later tasks will run `tuist setup cache` afterward.
9. Treat `mise run warm-external-cache` as the default post-init local verification/setup step once the initializer and templates exist, then commit/push if requested.
