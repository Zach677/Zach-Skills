...
def load_extend_settings(skill_name: str = "zach-wechat-hot-writer") -> dict[str, Any]:
...
def resolve_writer_preferences(skill_name: str = "zach-wechat-hot-writer") -> dict[str, Any]:
...
def estimate_reader_relevance(title: str, category: str) -> float:
    ...
    identity_hits = keyword_hits(combined, IDENTITY_SAFETY_KEYWORDS)
...
def build_angle_candidates(title: str) -> list[str]:
    ...
    elif any(keyword in title for keyword in IDENTITY_SAFETY_KEYWORDS):
...
def suggest_keywords_for_topic(topic: dict[str, Any]) -> list[str]:
    ...
    elif keyword_hits(combined, IDENTITY_SAFETY_KEYWORDS) > 0:
        keywords.extend(["家庭防骗", "个人信息安全", "官方核对", "父母"])
